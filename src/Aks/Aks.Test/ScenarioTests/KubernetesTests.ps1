...
function Test-NewAzAks
{
    ...
    try
    {
        ...
        #     -NodeName $nodeName -NodeOsType $nodeOsType -NodeCount $nodeCount -NodeOsDiskSize $nodeDiskSize
        New-AzAks -ResourceGroupName $resourceGroupName -Name $kubeClusterName -NetworkPlugin $networkPlugin `
            -KubernetesVersion $kubeVersion -EnableRbac -LoadBalancerSku $loadBalancerSku -LinuxProfileAdminUserName $linuxAdminUser -DnsNamePrefix $dnsNamePrefix
        New-AzAksNodePool -ResourceGroupName $resourceGroupName -ClusterName $kubeClusterName -Name $nodePoolName -OsType $nodeOsType -Count $nodeCount -OsDiskSize $nodeDiskSize -VmSize $nodeVmSize `
            -MaxCount $nodeMaxCount -MinCount $nodeMinCount -MaxPodCount $maxPodCount -NodeSetPriority Regular -EnableAutoScaling Deallocate -VmSetType VirtualMachineScaleSets
        # if (IsLive) {
        #     $cred = $(createTestCredential "Unicorns" "Puppies")
